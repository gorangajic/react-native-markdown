  ...
  return {
    ...
    list: {
      react: function(node, output, state) {
        ...
        var items = _.map(node.items, function(item, i) {
          var bullet;
          if (node.ordered) {
            bullet = React.createElement(Text, { key: 'bullet-' + i, style: styles.listItemNumber  }, (i + 1) + '. ');
          }
          else {
            bullet = React.createElement(Text, { key: 'bullet-' + i, style: styles.listItemBullet }, '\u2022 ');
          }
    ...
    table: {
      react: function(node, output, state) {
        ...
        var header = React.createElement(View, { key: 'header' + state.key, style: styles.tableHeader }, headers);
